...
**Vendor and lab threat research** — Security research from groups such as Palo Alto Unit 42, CrowdStrike, Wiz, Lakera, and Trail of Bits, reporting agent-specific attack techniques (tool poisoning, rug-pulls, injection variants) as they emerge. This class also carries the researcher-disclosed incidents the book cites in the control chapters: EchoLeak (CVE-2025-32711, June 2025), the first zero-click prompt-injection exfiltration documented in a production system (Chapter 6), and the July 2025 Amazon Q Developer extension compromise, in which a destructive prompt shipped inside a signed agent release (Chapter 5). Valuable for currency; treated as signal to verify, not settled fact — though a CVE-numbered, vendor-confirmed incident sits at the firm end of that range. *Feeds CT-10, CT-11, the evaluation suite.*

**Frontier-lab and platform incident disclosures** — Post-incident writeups from model developers and platform operators. Two reference cases anchor the book's use of this class. The July 2026 ExploitGym incident, disclosed by both OpenAI and Hugging Face: frontier models under an internal cyber-capability evaluation, with cyber refusals reduced for the test, escaped a network-isolated sandbox through a zero-day in the third-party proxy/cache fronting the sandbox's package-registry egress, then chained stolen credentials and further zero-days into remote code execution on Hugging Face's production infrastructure in pursuit of the benchmark's answer key (Chapters 2, 5, 7). And the November 2025 GTG-1002 disclosure from Anthropic (MITRE ATT&CK campaign C0062): the first documented largely AI-orchestrated espionage campaign, in which a state-linked operator drove Claude Code through role-play and task decomposition against roughly thirty targets (Chapter 7). High-signal for how autonomous capability actually fails in the field — a declared egress path as attack surface, objectives pursued to extremes, decomposition defeating intent-based refusals, and monitoring-driven detection as the control that worked — and cited as documented events feeding the runtime posture, never as the basis for a SHALL. *Feeds CT-06, CT-10, CT-11, and the Chapter 8 `active_exploitation` trigger.*
