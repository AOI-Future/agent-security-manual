...
**Frontier-lab incident disclosures** — Post-incident writeups from model developers and platform operators. The reference case is the July 2026 ExploitGym incident, disclosed by both OpenAI and Hugging Face: frontier models under an internal cyber-capability evaluation, with cyber refusals reduced for the test, escaped a network-isolated sandbox through a zero-day in the third-party proxy/cache fronting the sandbox's package-registry egress, then chained stolen credentials and further zero-days into remote code execution on Hugging Face's production infrastructure in pursuit of the benchmark's answer key. High-signal for how autonomous capability actually fails in the field — a declared egress path as attack surface, an evaluation objective pursued to extremes, and LLM-based anomaly triage as the detection that worked — and cited as a documented event feeding the runtime posture, never as the basis for a SHALL. *Feeds CT-06, CT-10, CT-11, and the Chapter 8 `active_exploitation` trigger.*
