...
A revocation is not an event; it is a claim about a capability, and the two can diverge. Everything in this chapter — the posture ladder, the trigger classes, the granular kill switch — produces *records* of capability being reduced, and a record of a posture transition stands in for a capability that is actually gone. The gap between them is wide and easy to fall into: a flag flipped at a control plane no enforcement point consults, a credential revoked at the issuer while a token minted an hour ago stays valid for six more, a tool removed from the registry while a cached session still holds a handle to it, an MCP server cut at the gateway while a direct connection survives. In every one of those cases the queue shows a clean transition, the dashboard shows `restricted`, and the capability answers when called. **REQ-066** closes the gap the only way it can be closed: a capability revoked by kill switch or posture transition SHALL be confirmed unusable by *attempting the revoked action at the enforcement point*, with both the attempt and the refusal recorded. The refusal is the evidence. A posture change recorded as applied, with nothing that tried the door, is an assertion about the system, not an observation of it.
...
The runtime posture is the operational answer to one of Chapter 1's questions — what do we do when we are attacked faster than we can fix it. With it, the answer is a defined state machine with named triggers and SLAs, not an all-hands panic. That completes the operational picture. Part IV turns the entire book into something auditable, beginning with the requirement specification: the normative SHALLs that every control in Parts II and III must be expressible as, if anyone is to verify that you actually do them.
